import AttributeCard from '@/components/AttributeCard'
import Title from '@/components/Title'

import {
  MagnifyingGlass,
  Repeat,
  RocketLaunch,
  Target,
} from '@phosphor-icons/react/dist/ssr'
import Image from 'next/image'
import Link from 'next/link'

export default function Services() {
  return (
    <section>
      <div className="relative">
        <div className="max-w-[1400px] mx-auto py-[80px] max-2xl:px-4">
          <div className="max-w-[875px]">
            <Title>Estrutura</Title>
            <h1 className="text-5xl mb-6 max-md:text-3xl text-slate900">
              Como funciona nosso <br />
              <span className="font-bold italic">processo</span> de{' '}
              <span className="font-bold italic">Transição Profissional?</span>
            </h1>
            <p className="text-gray-500 mb-9">
              Na Recruta F&C, apoiamos profissionais em momentos de transição de
              carreira, transformando mudanças em decisões conscientes e bem
              direcionadas. Atuamos no desenvolvimento de novas habilidades, no
              reposicionamento profissional e na conexão com oportunidades que
              realmente fazem sentido para cada perfil. O resultado é uma
              transição segura, consistente e com perspectivas reais de
              crescimento.
            </p>
            <Link
              href="https://wa.me/5516996093789/?text=Olá, gostaria de saber mais sobre Transição Profissional."
              target="_blank"
              className="block text-slate900 font-bold underline mb-[80px] w-fit hover:text-slate500 delay-150 ease-in-out transition-colors"
            >
              Fale com um especialista
            </Link>
          </div>

          <div className="grid grid-cols-4 gap-4 max-lg:grid-cols-2 max-sm:grid-cols-1">
            <AttributeCard
              text="Identificamos seus pontos fortes, experiências e objetivos para criar uma rota personalizada para sua próxima conquista."
              title="Mapeamento de Potencial"
              icon={<MagnifyingGlass size={32} />}
            />
            <AttributeCard
              text="Aprimoramos suas competências e adicionamos novas ferramentas ao seu arsenal para você se destacar no mercado."
              title="Upgrade de Habilidades"
              icon={<RocketLaunch size={32} />}
            />
            <AttributeCard
              text="Definimos juntos o caminho mais assertivo, conectando você a oportunidades que realmente fazem sentido."
              title="Direcionamento Estratégico"
              icon={<Target size={32} />}
            />
            <AttributeCard
              text="Seguimos lado a lado para garantir que sua transição seja ágil, fluida e com resultados reais."
              title="Acompanhamento de Virada"
              icon={<Repeat size={32} />}
            />
          </div>
        </div>
        <span className="absolute bottom-0 w-full h-[160px] bg-slate900 -z-10">
          <div className="max-w-[1400px] mx-auto relative">
            <Image
              src="/recolocacao/1.avif"
              width={502}
              height={507}
              alt="image"
              className="absolute right-0 bottom-0 max-xl:hidden"
            />
          </div>
        </span>
        <span className="absolute bottom-0 w-full h-full bg-gradient-to-t from-slate50 to-slate400 -z-30"></span>
      </div>
    </section>
  )
}
